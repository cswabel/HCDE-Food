...
class FoodManagementApp(tk.Tk):
    ...
    def onboarding_screen(self):
        ...
        if self.current_step < len(self.onboarding_steps):
            tk.Label(
                self,
                text=self.onboarding_steps[self.current_step],
            ...
            tk.Button(
                ...
                text="Next",
                font=("Helvetica", 14),
                command=self.next_onboarding_step,
                width=20,
                height=2,
                bg="#8e94f2",
                fg="#ffffff",
                activebackground="#5469b6",
                relief="flat",
            ).pack(pady=20)
        else:
            self.home_screen()
    ...
    def home_screen(self):
        ...
        tk.Label(
            self,
            text="Welcome to the Food Management App",
            font=("Helvetica", 20, "bold"),
            bg="#cdd3df",
            fg="#000000",
        ).pack(pady=30)
    ...
    def scan_barcode_expiry_date_screen(self):
        ...
        self.result_label = tk.Label(
            self, text="", font=("Helvetica", 12), bg="#cdd3df", fg="#23395b"
        )
    ...
    def ai_meal_generator_screen(self):
        ...
        tk.Button(
            scroll_frame,
            text="Generate Meal",
            command=self.generate_meal_based_on_selection,
            width=20,
            height=2,
            bg="#8e94f2",
            fg="#ffffff",
        ...
        self.generated_meal_label = tk.Label(
            scroll_frame, text="", font=("Helvetica", 12), bg="#cdd3df", fg="#23395b"
        )
    ...
    def generate_meal_based_on_selection(self):
        ...
        selected = [
            ingredient for ingredient, var in self.selected_ingredients if var.get()
        ]
        possible_meals = [
            meal
            for meal, ingredients in self.meal_pool.items()
            if all(item in selected for item in ingredients)
        ]
        if possible_meals:
            meal = random.choice(possible_meals)
            self.generated_meal_label.config(
                text=f"Suggested Meal: {meal}\nIngredients: {', '.join(self.meal_pool[meal])}"
            )
        else:
            self.generated_meal_label.config(
                text="No meals can be generated with the selected ingredients."
            )
    ...
    def community_engagement_screen(self):
        ...
        tk.Label(
            scroll_frame,
            text="Near-Date Items Available for Trade:",
            font=("Helvetica", 14),
            bg="#cdd3df",
            fg="#23395b",
        ).pack(pady=10)
        ...
        tk.Label(
            scroll_frame,
            text="You have earned credits for sharing items.",
            font=("Helvetica", 12),
            bg="#cdd3df",
            fg="#23395b",
        ).pack(pady=10)
    ...
    def voice_commands_screen(self):
        ...
        tk.Label(
            self,
            text="Voice Commands Activated. Please speak your command.",
            font=("Helvetica", 12),
            bg="#cdd3df",
            fg="#23395b",
        ).pack(pady=10)
        ...
        tk.Button(
            self,
            text="Execute",
            command=lambda: self.execute_voice_command(command.get()),
            width=20,
            height=2,
            bg="#8e94f2",
            fg="#ffffff",
    ...
    def profile_screen(self):
        ...
        tk.Label(
            self, text="Name:", font=("Helvetica", 12), bg="#cdd3df", fg="#23395b"
        ).pack(pady=10)
        self.name_entry = tk.Entry(
            self, font=("Helvetica", 12), bg="#ffffff", fg="#23395b"
        )
        self.name_entry.pack(pady=10)
        tk.Label(
            self, text="Email:", font=("Helvetica", 12), bg="#cdd3df", fg="#23395b"
        ).pack(pady=10)
    ...
    def dietary_preferences_screen(self):
        ...
        tk.Label(
            self,
            text="Dietary Preferences and Allergies",
            font=("Helvetica", 16, "bold"),
            bg="#cdd3df",
            fg="#000000",
        ).pack(pady=20)
        ...
        self.dietary_preferences_entry = tk.Entry(
            self, font=("Helvetica", 12), bg="#ffffff", fg="#23395b"
        )
        self.dietary_preferences_entry.pack(pady=10)
        tk.Label(
            self, text="Allergies:", font=("Helvetica", 12), bg="#cdd3df", fg="#23395b"
        ).pack(pady=10)
        self.allergies_entry = tk.Entry(
            self, font=("Helvetica", 12), bg="#ffffff", fg="#23395b"
        )
        self.allergies_entry.pack(pady=10)
        tk.Button(
            self,
            text="Save",
            command=self.save_dietary_preferences,
            width=20,
            height=2,
            bg="#8e94f2",
            fg="#ffffff",
    ...
    def save_dietary_preferences(self):
        """Save the user's dietary preferences and allergies."""
        dietary_preferences = self.dietary_preferences_entry.get()
        allergies = self.allergies_entry.get()
        messagebox.showinfo(
            "Preferences Saved",
            "Your dietary preferences and allergies have been updated.",
        )
    ...
    def manage_shopping_list_screen(self):
        ...
        for item in shopping_list:
            tk.Label(
                self, text=item, font=("Helvetica", 12), bg="#cdd3df", fg="#23395b"
            ).pack(pady=2)
        ...
        self.shopping_list_entry = tk.Entry(
            self, font=("Helvetica", 12), bg="#ffffff", fg="#23395b"
        )
        self.shopping_list_entry.pack(pady=10)
